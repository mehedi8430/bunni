...
export default function PieChartCard({
  ...
  useEffect(() => {
    if (customerData.length > 0 && !selectedDate) {
      ...
      const hasDataForToday = customerData.some(
        (customer) => customer.date === formattedToday,
      );
  ...
  const totalAmount: number = useMemo(() => {
    return filteredCustomers.reduce(
      (sum: number, customer: Customer) => sum + customer.amount,
      0,
    );
  }, [filteredCustomers]);
      ...
      <div className="mb-2 flex flex-col md:flex-row items-center justify-between space-y-2">
        ...
        <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
          <PopoverTrigger asChild>
            <Button
              variant={"outline"}
              className="border-sidebar-border cursor-pointer rounded-lg bg-white font-normal"
            >
          ...
          <PopoverContent className="w-auto p-0">
            <Calendar
              ...
              onSelect={(date: Date | undefined) => {
                // date can be undefined if cleared
                if (date) {
          ...
          <ul className="space-y-3">
            {filteredCustomers.length > 0 ? (
              ...
              <li className="py-4 text-center text-gray-500">
                No top customers for this date.
              </li>
            )}
          </ul>
        </div>
